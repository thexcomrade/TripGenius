...
class Settings(BaseSettings):
    ...
    DATABASE_URL: str = Field(
        default="sqlite:///./tripgenius.db"
    )

    TOURISM_DATASET_PATH: str = Field(
        default="database/tourism.csv"
    )
    ...
    def tourism_dataset_path(self) -> Path:
        ...
        if not dataset_path.exists():
            raise FileNotFoundError(
                f"Tourism dataset not found: {dataset_path}"
            )
    ...
    def sqlite_database_file(self) -> Path:
        if self.DATABASE_URL.startswith("sqlite:///"):
            database_name = self.DATABASE_URL.replace(
                "sqlite:///",
                ""
            )
